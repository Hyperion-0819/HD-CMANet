...
class PSRT(nn.Module):
    def __init__(self, arch, scale_ratio, n_select_bands, n_bands):
        ...
        self.w = Block(out_num=2, inside_num=3, img_size=self.img_size, in_chans=self.in_channels + n_select_bands, embed_dim=self.embed, head=8,
                       win_size=8)
        self.visual_corresponding_name = {}
        init_weights(self.conv)
        init_w(self.w)

    def forward(self, lms, rgb):
        '''
        :param rgb:
        :param lms:
        :return:
        '''
        lms = nn.functional.upsample(lms, scale_factor=self.scale_ratio, mode='bicubic')
        self.rgb = rgb
        self.lms = lms
